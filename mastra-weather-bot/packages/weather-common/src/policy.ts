import { dressRecommendationSchema, recommendationSignalsSchema, type DressRecommendation, type RecommendationSignals, type UserPreferenceProfile, type WeatherSnapshot } from './domain';
...
export function deriveRecommendationSignals(snapshot: WeatherSnapshot, preferences?: Partial<UserPreferenceProfile>): RecommendationSignals {
  ...
  const signals = recommendationSignalsSchema.parse({
    thermalLevel,
    rainLevel,
    windLevel,
    umbrellaAdvice,
  ...
  console.log('[POLICY] deriveRecommendationSignals →', JSON.stringify(signals, null, 2));
...
export function createDeterministicRecommendation(
  ...
  const recommendation = dressRecommendationSchema.parse({
  ...
  console.log('[POLICY] createDeterministicRecommendation →', JSON.stringify(recommendation.clothing, null, 2));
